                                       ...
                                       __in uint32_t max_msg_capacity) 
{
    ...
    msg_queue_p->ring_buf_p = ring_buffer_create_with_mem(raw_mem + sizeof(struct _fixed_msg_queue_s),
...
bool fixed_msg_queue_push(__in fixed_msg_queue msg_queue, __in const void *msg_p)
{
    ...
    return ring_buffer_write(msg_queue->ring_buf_p, msg_p,
...
bool fixed_msg_queue_pop(__in fixed_msg_queue msg_queue, __out void* msg_p)
{
    ...
    return ring_buffer_read(msg_queue->ring_buf_p, msg_p,
                           msg_queue->one_msg_byte_size) ==
           msg_queue->one_msg_byte_size;
}
...
void fixed_msg_queue_destroy(__inout fixed_msg_queue *msg_queue_p) 
{
    ...
    ring_buffer_destroy(&(msg_queue->ring_buf_p));
    free(msg_queue);
    *msg_queue_p = NULL;
}
