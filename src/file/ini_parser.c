...
typedef struct
{
	...
	stringbuilder_t *sb; // <-- hold dump ini string
} dump_ini_context;
...
{
	return (0 == size) ? NULL : malloc(size);
}

static inline void *allocator_my_calloc(size_t item_count, size_t item_size)
{
	return (0 == item_count || 0 == item_size) ? NULL : calloc(item_count, item_size);
}
...
)
{
	...
	{
		return 1; // just continue to parse file
	}
	char section_buf[SECTION_NAME_MAX_SIZE];
	char key_buf[SECTION_NAME_MAX_SIZE];
	trim_whitespace_copy(section, section_buf, sizeof(section_buf));
	trim_whitespace_copy(key, key_buf, sizeof(key_buf));
	if ('\0' == section_buf[0] || '\0' == key_buf[0])
	{
		return 1;
	}
	ini_parser_handle parser_p = (ini_parser_handle)user;
	if (INI_PARSER_CODE_SUCCEED != ini_parser_put_string(parser_p, section_buf, key_buf, value))
	{
		LOGE_TRACE("failed to put %s=%s to ini_parser", key, value);
		return 0;
	}
	return 1;
}
...
static bool iter_foreach_section(void *data, void *context)
{
	...
	if (NULL == context_p->p_section_info || NULL == context_p->p_section_info->plist_section || 
		'\0' == context_p->p_section_info->section_name[0])
	{
		return true; // just ignored empty section and continue
	}

	list_node_t *stop_node = list_foreach(context_p->p_section_info->plist_section,
										  iter_foreach_section_key_value, context_p);
	return (NULL == stop_node);
}
											 ...
											 const char *section, const size_t section_len)
{
	...
	if (!auto_create && (0 == list_length(parser_p->plist_sections)))
	{
		return NULL;
	}
	...
	if (auto_create && !target_section)
	{
		...
		{
			list_free(target_section->plist_section);
			allocator_my_free(target_section);
			return NULL;
		}
...
ini_parser_code_e ini_parser_put_string(ini_parser_handle parser_p,
										const char *section, const char *key, const char *value)
{
	if (!parser_p || !section || '\0' == section[0] || !key || '\0' == key[0])
	{
		return INI_PARSER_CODE_INVALID_PARAM;
	}
	char section_buf[SECTION_NAME_MAX_SIZE];
												   ...
												   const char *section, const char *key)
{
	...
	section_info_t *p_section = search_target_section(parser_p, false, section_buf, strlen(section_buf));
	if (!p_section)
	{
		return INI_PARSER_CODE_NOT_FOUND_SECTION_KEY;
	}
	key_value_t *p_kv = iter_section_for_search_target_kv(p_section->plist_section, key_buf);
	if (!p_kv)
	{
		return INI_PARSER_CODE_NOT_FOUND_SECTION_KEY;
	}
...
ini_parser_code_e ini_parser_delete_section(ini_parser_handle parser_p, const char *section)
{
	...
	if (parser_p->p_last_used_section == p_section)
	{
		parser_p->p_last_used_section = NULL;
	}
...
#define STR2NUM_TYPE(type, trans_func)             \
	type result = trans_func(str_value, &end_ptr); \
