...
static test_case_t g_all_test_cases[] =
	{
		...
		{file_logger_test, "test file logger"},
...
static int get_testcase_from_kb(int *p_testcases, int test_case_size)
{
	...
	while (case_count < test_case_size)
	{
		...
		++case_count;
		if ((NULL == p_loc) || (str_start = p_loc + 1) >= p_str_end)
		{
...
static void show_menu_test_case()
{
	...
	for (size_t i = 0; (buffer_left_len = sizeof(str_menu) - 1 - str_len) > 0 && i < (sizeof(g_all_test_cases) / sizeof(g_all_test_cases[0])); ++i)
	{
		str_len += snprintf(str_menu + str_len, buffer_left_len, "  %02zu : %s\n", i, g_all_test_cases[i].description);
	}
	fprintf(stderr, "input test case numbers (split by space),\n press enter to submit task:\n%s\n", str_menu);
}

static int run_specified_test_cases(const int *test_cases, int test_case_count)
{
	if (NULL == test_cases || test_case_count <= 0)
	{
		return 0;
	}
	int ret = 0;
	const int all_available_case_count = sizeof(g_all_test_cases) / sizeof(g_all_test_cases[0]);
	for (int i = 0; i < test_case_count; ++i)
	{
		int case_number = test_cases[i];
		if (case_number < 0 || case_number >= all_available_case_count)
		{
			fprintf(stderr, "invalid test case index: %d (valid range 0-%d)\n", case_number, all_available_case_count - 1);
			return -1;
		}
		test_case_t *p_case = &g_all_test_cases[case_number];
		fprintf(stderr, "\n\n==> Now Run testcase[%d]: %s\n", i, p_case->description);

		LOGI("==> Now Run testcase[%d]: %s\n", i, p_case->description);
		ret = p_case->test_fn();
		LOGI("<== End Run testcase[%d]: %s, ret=%d\n", i, p_case->description, ret);
...
static int run_test_case_from_user()
{
	int test_cases[64] = {-1};
	show_menu_test_case();
	int test_case_count = get_testcase_from_kb(test_cases, sizeof(test_cases) / sizeof(test_cases[0]));
	...
	return run_specified_test_cases(test_cases, test_case_count);
}

static int parse_test_cases_from_argv(int argc, char *argv[], int *out_cases, int max_cases)
{
	if (NULL == out_cases || max_cases <= 0)
	{
		return 0;
	}
	int count = 0;
	for (int i = 1; i < argc && count < max_cases; ++i)
	{
		...
		while (*cursor != '\0' && count < max_cases)
		{
			char *end_ptr = NULL;
			long value = strtol(cursor, &end_ptr, 10);
			if (end_ptr == cursor)
			{
				fprintf(stderr, "invalid test case argument: %s\n", arg);
				return -1;
			}
			if (value < 0 || value > INT_MAX)
			{
				fprintf(stderr, "test case argument out of range: %ld\n", value);
				return -1;
			}
			out_cases[count++] = (int)value;
			if (*end_ptr == ',')
			{
				cursor = end_ptr + 1;
				continue;
...
static bool wait_for_user_interrupt(int timeout_seconds)
{
...
#if _WIN32
	LOGI("after %d seconds, it will run automatically. if you want to choose test case, just press any key", timeout_seconds);
	if (timeout_seconds == 0)
	{
		if (_kbhit())
		{
...
int main(int argc, char *argv[])
{
	...
	int test_cases_from_cli[64] = {-1};
	int cli_case_count = parse_test_cases_from_argv(argc, argv,
													test_cases_from_cli, sizeof(test_cases_from_cli) / sizeof(test_cases_from_cli[0]));
	if (cli_case_count < 0)
	{
		LOGE("invalid command line arguments");
		ret = 1;
		goto EXIT_CLEANUP;
	}
	...
	do
	{
		if (cli_case_count > 0)
		{
			ret = run_specified_test_cases(test_cases_from_cli, cli_case_count);
			break;
		}

		bool show_menu = wait_for_user_interrupt(KB_TIMEOUT);
		if (show_menu)
		{
