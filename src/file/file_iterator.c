...
int file_iterator_foreach(char *dir, file_iterator_handle_file_info_fn handler, void *user_data)
{
	...
	{
		LOGE("error on opendir(\"%s\"): %d", dir, errno);
		return 1;
	}
	...
	while (NULL != (direntp = readdir(dirp)))
	{
		...
		if (d_name_len < 3U)
		{
			if (1U == d_name_len && '.' == direntp->d_name[0]) // current dir
			{
				continue;
			}
			if (2U == d_name_len && '.' == direntp->d_name[0] && '.' == direntp->d_name[1]) // parent dir
			{
				continue;
			}
		}
