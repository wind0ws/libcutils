...
extern "C" {
	 ...
	 */
	uint32_t RingBuffer_available_read(__in const ring_buf ring_buf_p);
	 ...
	 */
	uint32_t RingBuffer_available_write(__in const ring_buf ring_buf_p);
	...
	/**
	 * discard specified size memory from RingBuffer to target: this function just like RingBuffer_read but NOT copy memory.
