...
#define LOG_TAG_THPOOL "thpool_test"

//===============================TEST POOL==========================================BEGIN
static void task1(void *param) {
	TLOGD(LOG_TAG_THPOOL, "Thread#%u working on task1\n", (unsigned int)gettid());
}

static void task2(void *param) {
	TLOGD(LOG_TAG_THPOOL, "Thread#%u working on task2\n", (unsigned int)gettid());
}

static int thpool_test_basic()
{
	TLOGD(LOG_TAG_THPOOL, "Making threadpool with 4 threads");
	threadpool thpool = thpool_init(4);

	TLOGD(LOG_TAG_THPOOL, "Adding 40 tasks to threadpool");
	int i;
	for (i = 0; i < 20; i++) {
	...
	TLOGI(LOG_TAG_THPOOL, "Waiting till all the jobs have completed");
	thpool_wait(thpool);
	TLOGD(LOG_TAG_THPOOL, "Killing threadpool");
...
static void worker_sleep_1(int* secs) {
	TLOGD(LOG_TAG_THPOOL, "%d in", gettid());
	sleep(*secs);
	TLOGD(LOG_TAG_THPOOL, "%d out", gettid());
}
...
#ifndef _WIN32 /* pause resume only works on linux, because pthread_kill is not  */

static void sleep_4_secs(void *parm) {
	const char* task_name = (const char *)parm;
	TLOGD_TRACE(LOG_TAG_THPOOL, "%s now exec on %d...", task_name, gettid());
	//sleep(4);
	for (int i=0;i<100;i++)
	{
		usleep(40000);
	...
	TLOGD_TRACE(LOG_TAG_THPOOL, "%s exec finshed on %d...",task_name, gettid());
}
...
static int thpool_test_pause_resume() {
	...
	TLOGD_TRACE(LOG_TAG_THPOOL, "thpool is paused, 2 work is added. now sleeping 3s...");
	sleep(3);
	TLOGD_TRACE(LOG_TAG_THPOOL, "awake after 3s... now resume thpool..");
