...
int base64_test()
{
	...
	size_t encode_buf_len = base64_encode_buf_size(plain_str_len);
	char* encode_buf = (char*)malloc(encode_buf_len);

	int ret;
	if ((ret = base64_encode(encode_buf, plain_utf8_str, plain_str_len)) < (int)plain_str_len)
	{
	...
	if ((ret = base64_decode(decode_buf, encode_buf)) != (int)plain_str_len)
	{
