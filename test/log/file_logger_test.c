...
int file_logger_test_begin()
{
	...
	strcpy(g_logger_ctx.logger_cfg.log_file_name_prefix, "lcu");
	//for multi thread support, we should protect file logger
	//if you call xlog only on one thread, no need lock at all.
	g_logger_ctx.logger_cfg.lock.acquire = (void*)&my_file_logger_lock;
	g_logger_ctx.logger_cfg.lock.release = (void*)&my_file_logger_unlock;

	pthread_mutex_init(&g_logger_ctx.log_mutex, NULL);
	g_logger_ctx.logger_cfg.lock.arg = (void*)&g_logger_ctx.log_mutex;
